...
for pi in pi_data_table:
    ...
    if not reachable:
        ...
        command = "ssh pi@{} \'set +H;echo -e \"username=mchimento\npassword=***\ndomain=top.orn.mpg.de\" | sudo tee /etc/.smbcredentials;mkdir /home/pi/mnt; echo -e \"//10.76.0.7/grpLucy_Videos_GRETI /home/pi/mnt cifs x-systemd.automount,credentials=/etc/.smbcredentials,uid=pi,gid=pi,vers=3.0 0 0\" | sudo tee -a /etc/fstab\'".format(pi[1])
 ...
 #this goes in pis in aviary, IP address for accessing server is different from tower
 #//10.76.0.7/grpLucy_Videos_GRETI /home/pi/mnt cifs x-systemd.automount,credentials=/etc/.smbcredentials,uid=pi,gid=pi,vers=3.0 0 0
